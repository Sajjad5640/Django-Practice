from django.shortcuts import render,redirect
from form_app.models import ProductModel
from form_app.forms import ProductForm
# Create your views here.
def home_page (req):

    return render (req,'home.html')

def product_list_page(req):
    product_data = ProductModel.objects.all()
    context = {
        'product_data': product_data

    }

    return render (req,"product_list.html",context=context)


def add_product_page(req):

    if req.method == "POST":
        form_data = ProductForm(req.POST,req.FILES)
        if form_data.is_valid():
            data = form_data.save(commit=False)
            data.total_amount = data.product_price * data.qty
            data.save()
            return redirect('product')


    form_data = ProductForm ()

    context = {
        'form_data' : form_data

    }

    return render (req,'add_product.html',context=context)

def product_detail_page(req,p_id):
    product_data = ProductModel.objects.get(id = p_id)
    context = {
        'product_data': product_data

    }

    return render(req, 'product_detail.html',context=context)

def delete_product(req,p_id):
    ProductModel.objects.get(id = p_id).delete()
    return redirect('product')


def edit_product_page(req,p_id):
    product_data = ProductModel.objects.get(id = p_id)
    if req.method == "POST":
        form_data = ProductForm(req.POST,req.FILES,instance=product_data)
        if form_data.is_valid():
            data = form_data.save(commit=False)
            data.total_amount = data.product_price * data.qty
            data.save()
            return redirect('product')



    form_data = ProductForm(instance=product_data)
    context = {
        'form_data' : form_data

    }

    return render (req,'update_product.html',context=context)